...
const antennaTipMat = new THREE.MeshStandardMaterial({ color: '#4fe3ff', roughness: 0.2, toneMapped: false });
...
const heartMat = new THREE.MeshBasicMaterial({ color: '#4fe3ff', toneMapped: false });
...
function AntennaNavbar({ leftItems, contactText, contactHref, contactTarget, ctaText, onCtaClick }) {
  ...
  return (
    <nav className="sticky top-0 z-50 w-full pt-8 px-8 pointer-events-none">
      <div className="w-full max-w-[1400px] mx-auto flex flex-col relative pointer-events-auto">
        <div className="flex flex-col lg:flex-row items-center justify-between relative gap-4 lg:gap-0">
          <div className="flex flex-wrap justify-center lg:justify-start items-center gap-2 sm:gap-3 z-20">
            {leftItems.map((item, idx) => (
              <a
                ...
                className="relative px-7 py-2.5 rounded-full bg-white text-black hover:bg-zinc-200 text-sm font-bold transition-all overflow-hidden shadow-[0_4px_14px_rgba(0,0,0,0.1)]"
              >
                {item.label}
                {hoveredIndex === idx && (
                  <motion.div
          ...
          <div className="hidden lg:flex absolute left-1/2 top-1/2 -translate-x-1/2 -translate-y-1/2 items-center justify-center pointer-events-auto cursor-pointer group z-10">
            <div className="relative flex items-center justify-center h-12 w-16">
              ...
              <div className="z-10 w-10 h-10 bg-black/5 border-2 border-black/10 backdrop-blur-md rounded-[12px] flex items-center justify-center shadow-[0_4px_20px_rgba(0,0,0,0.1)] transition-all duration-300 group-hover:bg-black/10">
                <div className="w-[70%] h-[60%] bg-[#0a0a0a] rounded-lg flex items-center justify-center gap-1.5 overflow-hidden shadow-[inset_0_2px_4px_rgba(0,0,0,0.8)]">
                  <div className="w-1.5 h-3 bg-[#4fe3ff] rounded-[2px] shadow-[0_0_8px_#4fe3ff] transition-transform duration-200 group-hover:scale-y-[0.2]" />
                  <div className="w-1.5 h-3 bg-[#4fe3ff] rounded-[2px] shadow-[0_0_8px_#4fe3ff] transition-transform duration-200 group-hover:scale-y-[0.2]" />
                </div>
              </div>
            </div>
          ...
          <div className="flex flex-wrap justify-center lg:justify-end items-center gap-2 sm:gap-3 w-full lg:w-auto mt-4 lg:mt-0 z-20">
            <a
              ...
              className="px-5 sm:px-7 py-2.5 rounded-full bg-white text-black hover:bg-zinc-200 text-xs sm:text-sm font-bold transition-all shadow-[0_4px_14px_rgba(0,0,0,0.1)]"
            >
              {contactText}
            </a>
            <button
              onClick={onCtaClick}
              className="px-5 sm:px-7 py-2.5 rounded-full bg-[#4fe3ff] text-black text-xs sm:text-sm font-black hover:bg-[#2fd3f0] transition-colors flex items-center gap-2 shadow-[0_0_20px_rgba(79,227,255,0.4)]"
            >
              {ctaText}
              <ArrowRight size={18} />
            </button>
        ...
        <motion.div style={{ opacity: lineOpacity }} className="w-full mt-6 border-b-2 border-dotted border-black/15" />
      </div>
    </nav>
  );
}
...
export function RobotHero({
  ...
  contactText = 'GitHub',
  contactHref = 'https://github.com',
  contactTarget = '_blank',
  ctaText = 'Launch Console',
  onCtaClick,
  headline = 'Root Cause Analysis',
  subtitle = 'Instantly match production errors against your incident history. Get root causes, severity ratings, and automated fixes in seconds.',
  color = '#c4c4c4',
  scale = 1,
  pantallaColor = '#4fe3ff',
  pantallaBrillo = 1.2,
    ...
    >
      ...
      <div className="absolute inset-0 z-20 pointer-events-none flex flex-col">
        ...
        <div className="relative w-full max-w-[1400px] mx-auto px-8 flex-1 flex flex-col">
          <div className="mt-auto flex flex-col items-start pb-16 w-full max-w-xl pointer-events-auto">
            <h1 className="text-5xl sm:text-6xl lg:text-7xl font-extrabold text-black tracking-tight leading-[1.05]">
              {headline}
            </h1>
